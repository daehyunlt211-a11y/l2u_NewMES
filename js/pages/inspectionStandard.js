// 검사기준관리 — 좌: 품목 / 우: 해당 품목의 검사기준 상세리스트(수입/출하)
...
export async function inspectionStandards(root) {
  const state = { code: null, search: '', type: '전체', items: [], tools: [], stds: [] };

  root.innerHTML = `
    <div class="page-head">
      <div class="page-head__text"><h1>검사기준관리</h1><p>품목을 선택하면 해당 품목의 검사기준이 표시됩니다. 정량적은 숫자 측정, 정성적은 OK/NG 판정.</p></div>
    ...
    <div style="display:grid;grid-template-columns:340px 1fr;gap:18px;align-items:start">
      <div class="card">
        ...
        <div id="is-list" style="max-height:64vh;overflow-y:auto"></div>
      </div>
  ...
  async function loadAll() {
    [state.items, state.tools, state.stds] = await Promise.all([
      ...
      db.all('inspection_standards', { sort: 'std_no' }).catch((e) => { throw e; }),
    ]);
  }
  const countOf = (code) => state.stds.filter(s => s.item_code === code).length;
  ...
  function renderDetail() {
    ...
    const all = state.stds.filter(s => s.item_code === state.code);
    const list = state.type === '전체' ? all : all.filter(s => (s.inspect_type || '수입검사') === state.type);
    const editor = root.querySelector('#is-detail');
    const chip = (t) => `<button class="chip ${state.type === t ? 'active' : ''}" data-t="${t}">${t}<span class="chip__count">${t === '전체' ? all.length : all.filter(s => (s.inspect_type || '수입검사') === t).length}</span></button>`;
    editor.innerHTML = `
      ...
      <div class="card__body">
        <div class="chips" style="margin-bottom:14px">${['전체', ...TYPES].map(chip).join('')}</div>
        <div class="table-wrap"><table class="grid">
          <thead><tr><th>기준번호</th><th class="center">검사유형</th><th>검사항목</th><th class="center">평가방법</th><th>기준/판정</th><th>검사방법</th><th class="center">사용</th><th class="center" style="width:88px">관리</th></tr></thead>
      ...
      return `<tr><td class="cell-code">${escapeHtml(r.std_no)}</td><td class="center">${badge(r.inspect_type || '수입검사', r.inspect_type === '출하검사' ? 'brand' : 'info')}</td>
        <td class="cell-strong">${escapeHtml(r.inspect_item || '')}</td><td class="center">${badge(r.eval_method || '정량적')}</td>
        <td>${std}</td><td>${escapeHtml(r.method || '')}</td><td class="center">${yesNo(r.use_yn)}</td>
    ...
    }).join('') : `<tr><td colspan="8"><div class="empty" style="padding:36px">${icon('inbox', 44)}<h4>등록된 검사기준이 없습니다</h4><p>[검사기준 등록]으로 추가하세요.</p></div></td></tr>`}</tbody>
        </table></div>
      </div>`;
    editor.querySelectorAll('[data-t]').forEach(b => b.onclick = () => { state.type = b.dataset.t; renderDetail(); });
    editor.querySelector('#is-add').onclick = () => openForm(null);
    editor.querySelectorAll('[data-edit]').forEach(b => b.onclick = () => openForm(all.find(r => r.id === b.dataset.edit)));
  ...
  function openForm(r) {
    ...
    body.innerHTML = `
      ...
      <div class="field"><label>검사유형 <span class="req">*</span></label>
        <select class="select" name="inspect_type">${TYPES.map(t => `<option ${(r ? v('inspect_type') : (state.type === '전체' ? '수입검사' : state.type)) === t ? 'selected' : ''}>${t}</option>`).join('')}</select></div>
    ...
    openModal({
      title: `검사기준 ${isEdit ? '수정' : '등록'} — ${escapeHtml(item.name || '')}`, body, wide: true,
      ...
      onMount: ({ footEl, close }) => {
        ...
        footEl.querySelector('[data-ok]').onclick = async () => {
          ...
          const payload = {
            inspect_type: g('inspect_type'), item_code: item.code, item_name: item.name,
          ...
          try {
            ...
            state.type = payload.inspect_type;
