...
export async function dashboard(root) {
  root.innerHTML = `
    ...
    <div class="grid-2" style="margin-top:18px">
      ...
      <div class="card"><div class="card__head">${icon('radio', 18)}<h3>RFID 최근 이동</h3><div class="spacer"></div><a class="btn btn--sm" href="#/rfid/trace">LOT 추적</a></div><div class="card__body" id="dash-rfid"></div></div>
    </div>`;
  ...
  // RFID 최근 이동
  (async () => {
    const el = root.querySelector('#dash-rfid'); if (!el) return;
    try {
      const evs = (await db.all('rfid_events', {})).sort((a, b) => String(b.event_time).localeCompare(String(a.event_time))).slice(0, 6);
      el.innerHTML = evs.length ? `<div class="flex-col">${evs.map(e => `
        <div class="flex between" style="padding:8px 12px;background:var(--surface-2);border-radius:10px">
          <div><b>${escapeHtml(e.gate || '')}</b><div class="muted">${escapeHtml(e.lot_no || e.tag_uid || '')}</div></div>
          ${badge(e.event_type || '', null)}</div>`).join('')}</div>` : empty('RFID 이동 이력이 없습니다');
    } catch { el.innerHTML = empty('RFID 데이터 없음 (마이그레이션 필요)'); }
  })();
}
