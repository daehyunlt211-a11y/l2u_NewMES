...
const SCREENS = [
  ...
  S({ id: 'insp-in', title: '수입검사', group: '품질관리', path: '/quality/incoming', type: 'list',
    purpose: '자재입고관리에서 입고완료된 입고 건 리스트를 대상으로, 행별 [수입검사] 버튼으로 검사기준 기반 검사를 진행합니다. 납품관리·출하검사와 동일한 리스트+버튼 방식.',
    process: ['입고완료 입고 표시(미검사)', '[수입검사] 버튼', '검사기준 항목 평가(정량/정성)', '합격/불합격 저장'],
    relations: [
      { field: '대상 리스트', from: '자재입고관리', desc: '입고완료된 입고 건만 표시' },
      { field: '검사기준', from: '검사기준관리(수입검사)', desc: '품목 기준 자동 로드' },
    ...
    feats: [['수입검사 버튼', '행별 [수입검사] → 검사기준 평가 → 합격/불합격 저장, 완료 시 [결과]로 항목별 결과 조회']],
    alerts: ['검사기준 없으면 수동 판정 선택 안내', '입고완료 건만 대상(입고대기는 미표시)', '기간검색은 입고일 기준'] }),
