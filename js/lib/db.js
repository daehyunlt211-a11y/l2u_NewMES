...
function computeView(name) {
  ...
  if (name === 'tool_stocks') {
    ...
    return tools.map(t => {
      const inQ = moves.filter(m => m.tool_code === t.code && m.move_type === '입고').reduce((s, m) => s + (+m.qty || 0), 0);
      const outQ = moves.filter(m => m.tool_code === t.code && m.move_type === '출고').reduce((s, m) => s + (+m.qty || 0), 0);
      const dQ = disp.filter(x => x.tool_code === t.code).reduce((s, x) => s + (+x.qty || 0), 0);
      return { id: t.code, tool_code: t.code, tool_name: t.name, tool_type: t.tool_type, safety_stock: t.safety_stock, in_qty: inQ, out_qty: outQ, disposal_qty: dQ, stock_qty: inQ - outQ - dQ };
    });
  }
  return [];
}
